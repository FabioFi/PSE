...
#define GAIN_VALUE 10  // amplifier value
     ...
     nvr, fmi2Integer * value )
{
    ...
    {
        //----------------------------------------------------
        //  Getting Integer Ports here
        //----------------------------------------------------

        switch ( vr[i] )  // Value References of the ports
        {
            case 0L:
     ...
     nvr, fmi2Boolean * value )
{
    ...
    for (i = 0; i < nvr; i = i + 1)
    {
        ...
        //----------------------------------------------------
        //  Getting Boolean Ports here
        //----------------------------------------------------
        switch ( vr[i] ){
            case 0L:

                    value[i] = comp -> data_ready;
     ...
     communicationStepSize, fmi2Boolean noSetFMUStatePriorToCurrentPoint )
{
    ...
    if(comp -> data_ready == fmi2True){

        comp -> result = comp -> data * GAIN_VALUE;
        comp -> result_ready = fmi2True;
    }
